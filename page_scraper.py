...
def get_page_links_and_next(listing_url: str):
    ...
    # Try specific selectors for blog platforms (Blogger, etc.)
        ...
        for article in soup.find_all("article"):
            for a in article.find_all("a", href=True):
                ...
                if href_looks_valid(href):
                    ...
                    break  # Just get the first link per article

    # Another fallback: look for links with /20 pattern (blog post dates)
    if not article_urls:
...
def href_looks_valid(href: str) -> bool:
    ...
    if not href:
        return False
    # Accept absolute URLs or paths that look like blog posts
    if href.startswith(("http://", "https://")):
        return "/search" not in href  # Skip pagination/search links
    if href.startswith("/"):
